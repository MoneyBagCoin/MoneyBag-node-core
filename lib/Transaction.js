...
Transaction.prototype.serialize = function serialize() {
  ...
  var timestamp = this.timestamp===undefined ? Math.floor(new Date().getTime() / 1000) : this.timestamp;
  var buf = new Buffer(4);
...
TransactionSignatureSerializer.prototype.serialize = function() {
  ...
  // Creditbit doesn't include nTime in the serialization for signatures
  // serialize nTime
...
Transaction.prototype.getStandardizedObject = function getStandardizedObject() {
  ...
  var totalSize = 8; // version + lock_time + timestamp
