...
var BlockRules = {
  ...
  checkPowFromTimestamp: 1445766637,
  lastPowVersion: 2
};
...
function Block(data) {
  ...
  this.txs = data.txs || [];
  this.signature = data.signature || util.EMPTY_BUFFER;
}
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  if (this.version > BlockRules.lastPowVersion) {
      var lastByte = parser.buffer(1);
      //log.info('lastByte ', lastByte);
      if (lastByte.length > 0 && lastByte.readUInt8(0) > 0) {
          this.signature = parser.buffer(lastByte.readUInt8(0));
          //log.info('this.signature ' + this.signature.toString('hex'));
      }
  }
};
...
Block.prototype.calcHash = function calcHash() {
  ...
  return util.x11(header, this.timestamp);
};
...
Block.prototype.checkBlock = function checkBlock(txs) {
  ...
  //if (this.timestamp > BlockRules.checkPowFromTimestamp)
...
Block.getBlockValue = function getBlockValue(height, network) {
  if (!network)
    network = 'livenet';
...
// placeholder
Block.prototype.createCoinstakeTx =
  function createCoinstakeTx(beneficiary) {
    var tx = new Transaction();
    return tx;
};
